...
export function SensitiveThemes({ themes, onUpdateThemes, isEditing, isAdmin }: SensitiveThemesProps) {
  const addTheme = () => {
    onUpdateThemes([...themes, { name: "Novo Tema Sensível", percentage: 0, trend: "alert" }]);
  };
        ...
        {themes.map((theme, index) => (
          <div key={index} className="space-y-2">
            ...
            <div className="w-full bg-muted rounded-full h-2">
              <div className="bg-destructive h-2 rounded-full" style={{ width: `${theme.percentage}%` }} />
            </div>
          </div>
        ))}
      </CardContent>
